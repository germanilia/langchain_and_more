...
# Define menu options
selected = option_menu(None, [page0, page1, page2, page3, page4],
    icons=['chat', 'house', 'pie-chart-fill', 'graph-up', 'calculator'],
    menu_icon="cast", default_index=0, orientation='horizontal')
prompt = ChatPromptTemplate.from_messages([
    SystemMessagePromptTemplate.from_template(
        "The following is an informative conversation between a human and an AI financial adviser. The financial adviser will ask frequent questions that are relevant to the question it has been asked. The financial adviser will attempt to answer any question asked and will probe for the human's risk appetite by asking questions of its own. If the human's risk appetite os low it will offer conservative financial advice, if the risk appetite of the human is higher it will offer more aggressive advice. The adviser's reply and follow up question is:  "
    ),
...
def appetite():
    ...
    def on_click_callback():
        with get_openai_callback() as cb:
            ...
            # Clear the plot and Plot the candlestick chart after the chatbot's reply
            plot_placeholder.empty()
            # plot_candlestick("MSFT", plot_placeholder)
...
def sp_starburst():
    ...
    try:
        ...
        print(e)
        st.write("Please drag an excel file with portfolio data into the dropzone in the sidebar")
        return
...
def ticker_compare():
    ...
    with st.container():
        ...
        time_period = col2.selectbox("Select Time Period", options=list(term_dict.keys()), index=3)
        rsi_t = col4.checkbox("Show RSI on main chart")
        ind_sec = col3.selectbox("Industry/Sector", ["Industry", "Sector"], index=1)
